...
# snap-validate-serviceNC}"
    ...
    echo -e "${SNAP_BLUE}==>${SNAP_NC} Checking plugin installation"
    
    if pacman -Qi snapper-limine-plugin &>/dev/null; then
        echo -e "${SNAP_GREEN}✓${SNAP_NC} snapper-limine-plugin is installed"
        pacman -Qi snapper-limine-plugin | grep "Version" | sed 's/^/  /'
    else
        echo -e "${SNAP_RED}✗${SNAP_NC} snapper-limine-plugin is NOT installed"
        echo -e "\n${SNAP_YELLOW}Install with:${SNAP_NC} paru -S snapper-limine-plugin"
        return 1
    fi
    
    # Check plugin script location
    echo -e "\n${SNAP_BLUE}==>${SNAP_NC} Checking plugin script"
    
    local plugin_script="/usr/share/limine/snapper-limine.sh"
    if [[ -f "$plugin_script" ]]; then
        echo -e "${SNAP_GREEN}✓${SNAP_NC} Plugin script exists: $plugin_script"
        ls -lh "$plugin_script" | sed 's/^/  /'
    else
        echo -e "${SNAP_RED}✗${SNAP_NC} Plugin script not found: $plugin_script"
    fi
